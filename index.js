...
const fetchApiRepositorios = async (value) => {
  const resposta = await fetch(
    `https://api.github.com/users/${value}/repos?per_page=10`
  );
  const dados = await resposta.json();
  return dados;
};
...
const fetchApiEventos = async (value) => {
  const resposta = await fetch(
    `https://api.github.com/users/${value}/events?per_page=10`
  );
  const dados = await resposta.json();
  return dados;
};
...
const atualizar = async (usuario) => {
  ...
  nome.innerHTML = `<h3>${
    usuarioData.name ?? "Usuário não tem um nome cadastrado :("
  }</h3>`;
  bio.innerHTML = `<h4>${
    usuarioData.bio ?? "Usuário não tem uma bio cadastrada :("
  }</h4>`;
  ...
  reposInfo.textContent =
    repositoriosData.length > 0
      ? repositoriosData
          .slice(0, 10)
          .map((repo) => repo.name)
          .join(", ")
  ...
  repositoriosData.forEach((repo) => {
    reposHtml += `<h4> Repositório:&nbsp;</h4>  
<p> ${repo.name}  </p>
